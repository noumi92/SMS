...
public class MapsHandler implements MapsHandlerInterface {
    ...
    @Override
    public void getLatLongByCity(String cityName, Context context, SignupPresenterInterface signupPresenter) {
        mGeocoder = new Geocoder(context);
        List<Address> addressList = null;
        try {
            addressList = mGeocoder.getFromLocationName(cityName,1);
            Address address = addressList.get(0);
            LatLng latLng = new LatLng(address.getLatitude(), address.getLongitude());
            signupPresenter.onLatLongByCityLoad(latLng);
        }catch (Exception e){
            signupPresenter.onQueryResult("Location fetch failed: " + e.getMessage());
        }
